...
/* --- MOCK DATA (Retained for UI, replaced where real data exists) --- */
const announcements = [
...
const Header = ({ user, isCheckedIn }) => (
    <div className="flex justify-between items-center mb-8">
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Dashboard</h1>
            <p className="text-gray-500 text-sm mt-1">Welcome back, {user?.name || 'Employee'}! Here's what's happening today.</p>
        </div>
        <div className="flex items-center gap-3">
            <div className={`px-3 py-1 ${isCheckedIn ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'} text-xs font-bold rounded-full flex items-center gap-1`}>
                <div className={`w-2 h-2 ${isCheckedIn ? 'bg-green-600' : 'bg-gray-400'} rounded-full`}></div>
...
const Dashboard = () => {
    ...
    // Construct activities list based on real state if possible, otherwise mock or mixed
    const activities = [
        ...(isCheckedIn ? [{ title: "Clock In", time: `${checkInTime || 'Just now'} • Today`, icon: "fiber_manual_record", color: "text-blue-600" }] : []),
        ...
        <main className="flex-1 bg-gray-50 h-full overflow-y-auto p-8 font-sans custom-scrollbar">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <StatCard
                    ...
                    icon="schedule"
                    iconColor="text-blue-600 bg-blue-50"
                    footer={isCheckedIn ? "On Time" : "Not In"}
                    footerColor={isCheckedIn ? "text-green-600" : "text-gray-500"}
                    subtext="Today"
                />
            </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            <QuickAction
                                icon={isCheckedIn ? "timer_off" : "timer"}
