...
const LeaveManagement = () => {
  const { leaves, loading, updateStatus } = useLeave();
  ...
  const filteredRequests = useMemo(() => {
    return leaves.filter(req => {
      const name = req.employeeName || req.name || 'Unknown';
  ...
  }, [leaves, searchTerm, statusFilter, deptFilter]);
                ...
                <tr key={request.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <img src={request.avatar || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-1.2.1&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"} alt="" className="w-9 h-9 rounded-full object-cover border border-gray-100" />
                      <div>
                        <p className="font-bold text-gray-900 text-sm">{request.employeeName || request.name}</p>
                        <p className="text-xs text-gray-400">{request.role || 'Employee'}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6 text-right">
                    {request.status === 'Pending' ? (
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleAction(request.id, 'Approved')} className="text-green-600 hover:text-green-800 text-xs font-bold px-2 py-1 rounded hover:bg-green-50">Approve</button>
                        <button onClick={() => handleAction(request.id, 'Rejected')} className="text-red-600 hover:text-red-800 text-xs font-bold px-2 py-1 rounded hover:bg-red-50">Reject</button>
                      </div>
                    ) : (
